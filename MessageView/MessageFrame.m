...
- (void)setMessage:(MessageModel *)message{
    ...
    // 3、计算ID位置
    _nameF = CGRectMake(_iconF.origin.x+_iconF.size.width + 10, iconY-5, ChatIconWH, 14);
    
    // 4、计算内容位置
    CGFloat contentX = CGRectGetMaxX(_iconF)+ChatMargin;
    CGFloat contentY = message.from == MessageFromMe ? iconY +3: ( _nameF.origin.y+_nameF.size.height + 3);
