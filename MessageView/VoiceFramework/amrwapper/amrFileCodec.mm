...
static void SkipToPCMAudioData(FILE* fpwave)
{
	NS_RIFFHEADER riff;
	NS_FMTBLOCK fmt;
	NS_XCHUNKHEADER chunk;
	NS_WAVEFORMATX wfx;
	int bDataBlock = 0;
	
	// 1. 读RIFF头
	fread(&riff, 1, sizeof(NS_RIFFHEADER), fpwave);
	
	// 2. 读FMT块 - 如果 fmt.nFmtSize>16 说明需要还有一个附属大小没有读
	fread(&chunk, 1, sizeof(NS_XCHUNKHEADER), fpwave);
	if ( chunk.nChunkSize>16 )
	{
		fread(&wfx, 1, sizeof(NS_WAVEFORMATX), fpwave);
	}
	else
	{
		memcpy(fmt.chFmtID, chunk.chChunkID, 4);
		fmt.nFmtSize = chunk.nChunkSize;
		fread(&fmt.wf, 1, sizeof(NS_WAVEFORMAT), fpwave);
	}
...
static void WriteWAVEFileHeader(FILE* fpwave, int nFrame)
{
	...
	+ sizeof(NS_XCHUNKHEADER)               // fmt 
	+ sizeof(NS_WAVEFORMATX)           // NS_WAVEFORMATX
	+ sizeof(NS_XCHUNKHEADER)               // DATA
	+ nFrame*160*sizeof(short);    //
	strcpy(tag, "WAVE");
	memcpy(riff.chRiffFormat, tag, 4);
	fwrite(&riff, 1, sizeof(NS_RIFFHEADER), fpwave);
	...
	chunk.nChunkSize = sizeof(NS_WAVEFORMATX);
	fwrite(&chunk, 1, sizeof(NS_XCHUNKHEADER), fpwave);
	memset(&wfx, 0, sizeof(NS_WAVEFORMATX));
	wfx.nFormatTag = 1;
	wfx.nChannels = 1; // 单声道
	...
	fwrite(&wfx, 1, sizeof(NS_WAVEFORMATX), fpwave);
